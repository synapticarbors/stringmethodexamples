...
for b in beta:
    fnames = glob('../wemd_{beta}/analysis/sim*.h5'.format(beta=b))
    nsims = len(fnames)
    H = np.zeros((nsims,nbins))

    for fi,f in enumerate(fnames):
        h5file = h5py.File(f,'r')
        h = h5file['dist']['data'][-lastN:,:].sum(0)
        h /= h.sum()
        H[fi,:] = h
        h5file.close()

    ax.semilogy(theta,H.mean(0),lw=0.5,ls='None',marker='o',ms=2,c='r',mfc='None')
    #ax.errorbar(theta, H.mean(0),yerr=2.0*H.std(0)/np.sqrt(10.0), lw=0.5,ls='None',marker='o',ms=2,c='r',mfc='None')
